...
theorem SSubtyp.dealias_right_cforall
  ...
  ∃ B1 E1, SType.Dealias Γ S1 (SType.cforall B1 E1) := by
  apply SSubtyp.rec
  ...
  case trans =>
    unfold dealias_right_cforall.smotive
    repeat intro
    rename_i ih1 ih2 B3 E3 ht3 hd3
    have ih2 := ih2 ht3 hd3
    have ⟨B2, E2, hd2⟩ := ih2
    have ih1 := ih1 ht3 hd2
    exact ih1
...
theorem SSubtyp.sub_dealias_cforall_inv
  ...
  (h2 : SType.Dealias Γ S2 (SType.cforall B2 E2))
  (hs : SSubtyp Γ S1 S2) :
  Subbound Γ B2 B1 ∧ ESubtyp (Γ.cvar (CBinding.bound B2)) E1 E2 := by
  apply SSubtyp.rec
...
theorem SSubtyp.dealias_right_label
  ...
  ∃ T1, SType.Dealias Γ S1 (SType.label T1) := by
  apply SSubtyp.rec
    (motive_1 := fun Γ E1 E2 h => SSubtyp.dealias_right_label.emotive Γ E1 E2)
    (motive_2 := fun Γ C1 C2 h => SSubtyp.dealias_right_label.cmotive Γ C1 C2)
    (motive_3 := fun Γ S1 S2 h => SSubtyp.dealias_right_label.smotive Γ S1 S2)
    (t := h) (hd := hd) (ht := ht)
  case exist =>
    unfold dealias_right_label.emotive dealias_right_label.cmotive
    aesop
  case type =>
    unfold dealias_right_label.emotive dealias_right_label.cmotive
    aesop
  case capt =>
    unfold dealias_right_label.cmotive
    aesop
  case top =>
    unfold dealias_right_label.smotive
    repeat intro
    rename_i hd
    cases hd
  case refl =>
  ...
  case tinstr =>
    ...
    { trivial }
  case boxed =>
    unfold dealias_right_label.cmotive dealias_right_label.smotive
    repeat intro
    rename_i _ T2 _ hd
    cases hd
  case xforall =>
    unfold dealias_right_label.emotive dealias_right_label.cmotive dealias_right_label.smotive
    repeat intro
    rename_i hd
    cases hd
  case tforall =>
    unfold dealias_right_label.smotive dealias_right_label.emotive
    repeat intro
    rename_i hd
    cases hd
  case cforall =>
...
theorem SSubtyp.sub_dealias_label_inv
  (ht : Γ.IsTight)
  (h1 : SType.Dealias Γ S1 (SType.label T1))
  (h2 : SType.Dealias Γ S2 (SType.label T2))
  (hs : SSubtyp Γ S1 S2) :
